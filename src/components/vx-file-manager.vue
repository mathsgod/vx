				...
				<div class="content-wrapper">
					...
					<div class="content-body">
						...
						<div class="file-manager-main-content">
							...
							<div
								class="file-manager-content-body"
								ref="fileContent"
							>
								...
								>
									<h6 class="files-section-title mt-25 mb-75">
										Folders
									</h6>
								...
								>
									...
									<vx-file-manager-file
										...
										@unselected="
											selectedFile = selectedFile.filter(
												(s) => s != $event
											)
										"
...
export default {
	...
	methods: {
		...
		addNew(command) {
			...
			if (command == "folder") {
				this.$prompt("Please input new folder name").then(
					async ({ value }) => {
						let { data } = await this.$vx.post(
		...
		async renameFile(data) {
			let resp = (await this.$vx.post("/FileManager/renameFile", data))
				.data;
			if (resp.error) {
		...
		async renameFolder(data) {
			let newNode = (
				await this.$vx.post("/FileManager/renameFolder", data)
			).data;
			this.reloadContent();
